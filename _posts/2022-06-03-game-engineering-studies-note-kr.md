...
{% include image.html src="/assets/images/GameEngineering/VectorDivision.png" alt="VectorDivision" caption="VectorDivision" %}

이때 **v'**를 또다른 벡터 **b**에 대해서도 회전을 해주면 **v''**를 구할 수 있음.

{% include image.html src="/assets/images/GameEngineering/Rotor.png" alt="Rotor" caption="Rotor" %}
...
{% include image.html src="/assets/images/GameEngineering/Complement.png" alt="Complement" caption="Complement" %}
...
{% include image.html src="/assets/images/GameEngineering/AngularVelocityFigure.png" alt="AngularVelocityFigure" caption="AngularVelocityFigure" %}
...
{% include image.html src="/assets/images/GameEngineering/AngularVelocityEquation.png" alt="AngularVelocityEquation" caption="AngularVelocityEquation" %}

각속도는 보통 회전축 **A**에 평행하면서 크기가 \|ω(t)\|인 벡터로 표기함. 즉 각속도 벡터 **ω***(t)는 다음과 같음:

{% include image.html src="/assets/images/GameEngineering/AngularVelocityVectorEquation.png" alt="AngularVelocityVectorEquation" caption="AngularVelocityVectorEquation" %}
...
{% include image.html src="/assets/images/GameEngineering/LinearVelocityFigure.png" alt="LinearVelocityFigure" caption="LinearVelocityFigure" %}
...
{% include image.html src="/assets/images/GameEngineering/DifferentialsOfYi.png" alt="DifferentialsOfYi" caption="DifferentialsOfYi" %}

위의 식을 벡터에 대해서 표현할 수 있음:

{% include image.html src="/assets/images/GameEngineering/VectorDifferentialsOfY.png" alt="VectorDifferentialsOfY" caption="VectorDifferentialsOfY" %}

이때 이 X에 대한 F의 편미분으로 구성된 행렬을 야코비라 부르며, 현재 x<sub>i</sub> 값들에 대한 함수라고 보면 됨. 마치 X의 속도를 정의역으로, Y의 속도를 치역으로 갖는 함수라고 생각하면 됨:

{% include image.html src="/assets/images/GameEngineering/JacobianMapping.png" alt="JacobianMapping" caption="JacobianMapping" %}
...
{% include image.html src="/assets/images/GameEngineering/EndEffectorY.png" alt="EndEffectorY" caption="EndEffectorY" %}

이런 식으로 표현할 수도 있는 것임.

IK의 경우 야코비는 곧 관절의 각속도 ![AngularVelocity](/assets/images/GameEngineering/AngularVelocity.png)를 end effector의 위치와 orientation ![EndEffectorPositionOrientation](/assets/images/GameEngineering/EndEffectorPositionOrientation.png)이 됨. 즉,

{% include image.html src="/assets/images/GameEngineering/JacobianEndEffector.png" alt="JacobianEndEffector" caption="JacobianEndEffector" %}

여기서 **V**는 선형 및 회전 속도로 구성된 벡터이므로, end effector의 희망 변화량을 의미하게 됨. 이 희망 변화량은 현재 위치/orientation과 목표 간의 차이에 기반하여 구하게 될 것임.

{% include image.html src="/assets/images/GameEngineering/VelocityVector.png" alt="VelocityVector" caption="VelocityVector" %}

![AngularVelocity](/assets/images/GameEngineering/AngularVelocity.png)는 각 관절의 속도, 혹은 각 관절의 변화량을 의미함. 이게 결국 구해야하는 값임:

{% include image.html src="/assets/images/GameEngineering/AngularVelocitiesVector.png" alt="AngularVelocitiesVector" caption="AngularVelocitiesVector" %}

야코비 행렬 J는 결국 뒤의 둘 사이의 관계를 의미하며, 현재 pose에 대한 함수임. 이 행렬은 다음과 같이 생겼을 것임:

{% include image.html src="/assets/images/GameEngineering/JacobianMatrix.png" alt="JacobianMatrix" caption="JacobianMatrix" %}
...
{% include image.html src="/assets/images/GameEngineering/AngularLinearVelocities.png" alt="AngularLinearVelocities" caption="AngularLinearVelocities" %}
...
{% include image.html src="/assets/images/GameEngineering/PlanarThreeJointManipulator.png" alt="PlanarThreeJointManipulator" caption="PlanarThreeJointManipulator" %}
...
{% include image.html src="/assets/images/GameEngineering/ChangesInPosition.png" alt="ChangesInPosition" caption="ChangesInPosition" %}

end effector에 가할 목표 변화량은 end effector의 현재 위치와 목표 위치 간의 차임 (선형 변화량. 선형 속도):

{% include image.html src="/assets/images/GameEngineering/DesiredChange.png" alt="DesiredChange" caption="DesiredChange" %}
...
{% include image.html src="/assets/images/GameEngineering/JointAngleChangesJacobian.png" alt="JointAngleChangesJacobian" caption="JointAngleChangesJacobian" %}

### 야코비 역행렬

야코비를 이제 구했으면

{% include image.html src="/assets/images/GameEngineering/JointAngleVelocities.png" alt="JointAngleVelocities" caption="JointAngleVelocities" %}

이 식을 해결해야함. 만약 J가 사각행렬이라면 역행렬 J<sup>-1</sub>을 구해서 end effector의 속도가 주어졌을 때 각 관절의 각속도를 구할 수 있음.

{% include image.html src="/assets/images/GameEngineering/JacobianInverse.png" alt="JacobianInverse" caption="JacobianInverse" %}
...
{% include image.html src="/assets/images/GameEngineering/SingularConfiguation.png" alt="SingularConfiguation" caption="SingularConfiguation" %}
...
{% include image.html src="/assets/images/GameEngineering/PseudoInverse.png" alt="PseudoInverse" caption="PseudoInverse" %}
...
{% include image.html src="/assets/images/GameEngineering/PseudoInverseMapping.png" alt="PseudoInverseMapping" caption="PseudoInverseMapping" %}

이제 위의 식에서 약간의 수정을 해준 다음, LU 분해를 통해 β에 대한 아래 방정식을 풀 수 있게 됨:

{% include image.html src="/assets/images/GameEngineering/LUDecomposedPseudoInverse.png" alt="LUDecomposedPseudoInverse" caption="LUDecomposedPseudoInverse" %}

이걸 이제 아래 식으로 수정해서 ![AngularVelocity](/assets/images/GameEngineering/AngularVelocity.png)를 구할 수 있음:

{% include image.html src="/assets/images/GameEngineering/SubstitutedVelocities.png" alt="SubstitutedVelocities" caption="SubstitutedVelocities" %}
...
{% include image.html src="/assets/images/GameEngineering/IK2dExample.png" alt="IK2dExample" caption="IK2dExample" %}
...
{% include image.html src="/assets/images/GameEngineering/DampedLeastSquaresApproach.png" alt="DampedLeastSquaresApproach" caption="DampedLeastSquaresApproach" %}
...
{% include image.html src="/assets/images/GameEngineering/PseudoInverseVsDampedLeastSquares.png" alt="PseudoInverseVsDampedLeastSquares" caption="PseudoInverseVsDampedLeastSquares" %}
...
{% include image.html src="/assets/images/GameEngineering/NewtonRaphsonTaylorSeries.png" alt="NewtonRaphsonTaylorSeries" caption="NewtonRaphsonTaylorSeries" %}
...
{% include image.html src="/assets/images/GameEngineering/NewtonRaphsonTaylorSeriesSecondOrder.png" alt="NewtonRaphsonTaylorSeriesSecondOrder" caption="NewtonRaphsonTaylorSeriesSecondOrder" %}
...
{% include image.html src="/assets/images/GameEngineering/NewtonRaphsonSingleValueUpdate.png" alt="NewtonRaphsonSingleValueUpdate" caption="NewtonRaphsonSingleValueUpdate" %}

이렇게만 했을 것임.

하지만 이제는 다변수 함수이므로:

{% include image.html src="/assets/images/GameEngineering/NewtonRaphsonMultivariateUpdate.png" alt="NewtonRaphsonMultivariateUpdate" caption="NewtonRaphsonMultivariateUpdate" %}
...
{% include image.html src="/assets/images/GameEngineering/QuasiNewtonCondition0.png" alt="QuasiNewtonCondition0" caption="QuasiNewtonCondition0" %}

위의 식을 행렬식으로 다시 작성하면:

{% include image.html src="/assets/images/GameEngineering/QuasiNewtonCondition1.png" alt="QuasiNewtonCondition1" caption="QuasiNewtonCondition1" %}

이제 기울기가 0인 지점을 구해야하니, ![GradientIsZero](/assets/images/GameEngineering/GradientIsZero.png)를 만족해야함. 즉,

{% include image.html src="/assets/images/GameEngineering/DeltaTheta.png" alt="DeltaTheta" caption="DeltaTheta" %}

할선 방정식 뿐만 아니라, 대부분의 경우 B는 대칭이며, 헤시안이랑 최대한 비슷해야함.

B를 ∇<sup>2</sup>F(θ<sub>n</sub>)로 근사할 때, B가 최대한 계산하기 쉬워야하고, ![DeltaTheta](/assets/images/GameEngineering/DeltaTheta.png)는 풀기 쉬워야 함.

### 유사 뉴턴 방법

일반적인 유사 뉴턴 방법의 순서:

1. ![DeltaTheta](/assets/images/GameEngineering/DeltaTheta.png) 풀기
...
{% include image.html src="/assets/images/GameEngineering/BFGSUpdate.png" alt="BFGSUpdate" caption="BFGSUpdate" %}

역행렬은 다음과 같이 업데이트 가능:

{% include image.html src="/assets/images/GameEngineering/BFGSInverseUpdate.png" alt="BFGSInverseUpdate" caption="BFGSInverseUpdate" %}

둘 다 O(n<sup>2</sup>) 정도의 비용이 들음.

만약 y<sup>T</sup>Δθ > 0이라면, BFGS는 B<sub>k</sub>의 양의 정부호성을 유지함. 다시 말하자면 ![DeltaTheta](/assets/images/GameEngineering/DeltaTheta.png)가 내려가는 방향이라는 것임. 이건 역 행렬 업데이트 공식으로 구할 수 있음:

{% include image.html src="/assets/images/GameEngineering/BFGSInverseUpdateRewritten.png" alt="BFGSInverseUpdateRewritten" caption="BFGSInverseUpdateRewritten" %}
...
{% include image.html src="/assets/images/GameEngineering/FiniteDifferencing.png" alt="FiniteDifferencing" caption="FiniteDifferencing" %}

이걸 다르게 재작성하면:

{% include image.html src="/assets/images/GameEngineering/FiniteDifferencingSecantMethod.png" alt="FiniteDifferencingSecantMethod" caption="FiniteDifferencingSecantMethod" %}
...
{% include image.html src="/assets/images/GameEngineering/CCDFigure.png" alt="CCDFigure" caption="CCDFigure" %}
...
{% include image.html src="/assets/images/GameEngineering/CCDAngleFigure.png" alt="CCDAngleFigure" caption="CCDAngleFigure" %}
...
{% include image.html src="/assets/images/GameEngineering/TriangulationIKFigure.png" alt="TriangulationIKFigure" caption="TriangulationIKFigure" %}
...
{% include image.html src="/assets/images/GameEngineering/FABRIKFigure0.png" alt="FABRIKFigure0" caption="FABRIKFigure0" %}

{% include image.html src="/assets/images/GameEngineering/FABRIKFigure1.png" alt="FABRIKFigure1" caption="FABRIKFigure1" %}

{% include image.html src="/assets/images/GameEngineering/FABRIKFigure2.png" alt="FABRIKFigure2" caption="FABRIKFigure2" %}
...
{% include image.html src="/assets/images/GameEngineering/FABRIKJointRestriction.png" alt="FABRIKJointRestriction" caption="FABRIKJointRestriction" %}
...
{% include image.html src="/assets/images/GameEngineering/FABRIKEllipsoidalShape.png" alt="FABRIKEllipsoidalShape" caption="FABRIKEllipsoidalShape" %}
...
{% include image.html src="/assets/images/GameEngineering/FABRIKEllipsoidalShapeFigure0.png" alt="FABRIKEllipsoidalShapeFigure0" caption="FABRIKEllipsoidalShapeFigure0" %}
