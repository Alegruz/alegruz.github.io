...
Ultimately, all you need is a four-component RGBA color as the result of the rendering process. Whatever happens inside the renderer blackbox is just details. But in order to really understand how a color is produced in rendering, one must understand two topics: **colormetry** and **radiometry**. The former deals with colors; how we perceive, define, and process color. The latter deals with lights; how lights travel, scatter, and transmit energy. ReSTIR is about illumination, and is not a paper on spectral rendering, so it is okay for us to just learn radiometry for now.
...
We need one more physical dimension to consider: **the time**. The amount of energy transferred/converted per unit time is called **power**, and is measured in watts(W). So, in theory, a 100W light bulb is omitting 100J of light energy per second. Of course, to be physically correct(no pun intended), due to some heat loss the resulting omission would be less than 100J, but say that it does omit 100J for simplicity. Let's say that we are using a light that omits &lambda; = 500 nm photons. According to the equation between photon energy and its wavelength:
...
Again, we are interested in the **spectral power**, measured in W(nm)<sup>-1</sup>. The standard symbol used to represent the power is &Phi;, and use &Phi;<sub>&lambda;</sub> to represent the spectral power.
...
Let's focus first on the incoming lights. When defining the amount of photons traveling into a specific point per time, we should again use the density. Given a finite area A, we can define the density of power over the area as E = &Phi; / A. This could be both **irradiance** (E), the density of lights comining into the area A, and **radiance exitance** (M), the density of lights exiting the area A. Beware, radiance exitance and radiance are totally different concepts!!

For example, say that there is a light bulb omitting lights to every spherical direction uniformly. If we put the radius of the sphere as r, then the area is 4&pi;r<sup>2</sup>, making the radiance exitance of a point p:
