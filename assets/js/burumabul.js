import { supabase, ensureAuth } from "./supabaseClient.js";
...
(function () {
      ...
      function applyBoardPolicy(policy, providedPath) {
        if (providedPath && providedPath.length === BOARD_TEMPLATE.length) {
          BOARD_PATH_IDS = providedPath.slice();
        } else {
      ...
      function collectSetupConfig(overrides) {
        overrides = overrides || {};
        var count = parseInt(elPlayerCount.value, 10) || 2;
        count = Math.min(4, Math.max(2, overrides.playerCount || count));
        var names = [
        ...
        if (overrides.playerNames) {
          names = overrides.playerNames.slice();
        }
        if (overrides.primaryName) {
          names[0] = overrides.primaryName;
        }
        var policyValue = overrides.tilePolicy;
        if (!policyValue && elTilePolicy) {
          policyValue = elTilePolicy.value || "vanilla";
        }
        return {
          playerCount: count,
          playerNames: names,
          tilePolicy: policyValue || "vanilla",
        };
      }

      function buildInitialGameState(config) {
        config = config || collectSetupConfig();
        var playerCount = Math.min(4, Math.max(2, config.playerCount || 2));
        var playerNames = config.playerNames
          ? config.playerNames.slice(0, playerCount)
          : collectSetupConfig({ playerCount: playerCount }).playerNames.slice(
              0,
              playerCount
            );
        var policyValue = config.tilePolicy || "vanilla";
        var boardPath = buildBoardForPolicy(policyValue);
        applyBoardPolicy(policyValue, boardPath);
        var colorIdx = shuffle([0, 1, 2, 3]).slice(0, playerCount);
        var startMoney = playerCount === 2 ? START_MONEY_2P : START_MONEY_3P;
        ...
        var nextState = {
          ...
          tileDetailIndex: null,
          lastArrivalIndex: null,
          boardPolicy: policyValue,
          boardPath: boardPath.slice(),
        };
      ...
      function setGameState(nextState, options) {
        ...
        options = options || {};
        gameState = nextState;
        var policy = gameState.boardPolicy || "vanilla";
        var path = gameState.boardPath || buildBoardForPolicy(policy);
        applyBoardPolicy(policy, path);
        if (!gameState.boardPath || gameState.boardPath.length !== path.length) {
          gameState.boardPath = BOARD_PATH_IDS.slice();
        }
      ...
      function updateOnlineStatusUI(customText) {
        if (!elOnlineStatus) return;
        if (customText) {
          elOnlineStatus.textContent = customText;
          return;
        }
        if (gameMode === "online" && currentRoom) {
          var code = currentRoom.code || "-";
          var names = currentRoomPlayers.length
            ? currentRoomPlayers.map(function (p) {
                return p.name;
              })
            : ["대기 중"];
          elOnlineStatus.textContent =
            "온라인 모드 · 방 " + code + " · " + names.join(", ");
        } else {
          elOnlineStatus.textContent = "로컬 모드 (Supabase 미사용)";
        }
      }
    ...
    function showGoldenKeyPopup(text, callback) {
      ...
      function drawGoldenCard() {
        if (gameState.goldenDeck.length === 0) {
          initGoldenDeck();
        }
        return gameState.goldenDeck.shift();
      }
      ...
      // ---------- Actions -------------------------------------------------
      function startGame() {
        var config = collectSetupConfig();
        var nextState = buildInitialGameState(config);
        setGameState(nextState, { render: false });
        initGoldenDeck();
        gameState.boardPath = BOARD_PATH_IDS.slice();
        ...
        renderAll();
        var policyLabel = tilePolicyLabel(config.tilePolicy);
        elMessage.textContent =
          "게임 시작! 타일 정책: " +
          (policyLabel || config.tilePolicy) +
          ". 첫 번째 플레이어가 주사위를 굴리세요.";
        logEvent(
          null,
          "타일 정책 적용: " + (policyLabel || config.tilePolicy),
          0
        );
      }
      ...
      function rollDice() {
        var p = currentPlayer();
        if (!p || gameState.gameOver) return false;
        if (p.bankrupt) {
          var passed = endTurn();
          checkGameOver();
          return !!passed;
        ...
        if (p.spacePending) return false; // 우주여행 먼저 처리해야 함
        ...
        if (gameState.hasRolled && gameState.extraRolls <= 0) {
          return false;
        }
      ...
      function attemptPurchase(boardIndex) {
        var p = currentPlayer();
        var tile = tileAt(boardIndex);
        if (!p || !tile || gameState.gameOver || p.bankrupt) return false;
        if (tile.type !== "city" && tile.type !== "vehicle") return false;
      ...
      function attemptBuild(boardIndex) {
        ...
        var tile = tileAt(boardIndex);
        if (!p || !tile || gameState.gameOver || p.bankrupt) return false;
        if (tile.type !== "city") return false;
      ...
      function endTurn() {
        var p = currentPlayer();
        if (!p || gameState.gameOver) return false;
        if (!gameState.hasRolled || p.spacePending) return false;
      ...
      function handleSpaceGo() {
        var p = currentPlayer();
        if (!p || !p.spacePending || gameState.gameOver) return false;
      ...
      async function refreshOnlinePlayers(roomId) {
        if (!roomId) {
          currentRoomPlayers = [];
          updateOnlineStatusUI();
          return;
        ...
        try {
          var res = await supabase
            ...
            .eq("room_id", roomId);
          if (res.error) {
            console.error("players fetch error", res.error);
            return;
          }
          currentRoomPlayers = res.data || [];
          updateOnlineStatusUI();
        } catch (err) {
        ...
        roomChannel = supabase
          ...
          .on(
            ...
            function (payload) {
              ...
              var nextState = payload.new.state;
              if (!nextState) return;
      ...
      function snapshotState(baseState) {
        var src = baseState || gameState;
        var clone = JSON.parse(JSON.stringify(src));
        clone.boardPath = BOARD_PATH_IDS.slice();
        return clone;
      }

      async function commitMove(patchState) {
        var payload = snapshotState(patchState || gameState);
        ...
        try {
          var res = await supabase
            ...
            .single();
          if (res.error) {
            console.error("commitMove error", res.error);
            alert("게임 상태를 저장하지 못했습니다. 다시 시도해주세요.");
            throw res.error;
          }
          currentRoom = res.data;
          return payload;
        } catch (err) {
          console.error("commitMove exception", err);
          alert("게임 상태 동기화 중 오류가 발생했습니다.");
          throw err;
        }
      }

      async function syncOnlineState(source) {
        if (gameMode !== "online" || !currentRoom) return;
        try {
          await commitMove(gameState);
          console.debug("[sync]", source || "action");
        } catch (err) {
          console.error("syncOnlineState", err);
        }
      }
      ...
      async function createRoom(lobbyCode, playerName) {
        ...
        try {
          var config = collectSetupConfig({ primaryName: playerName });
          var nextState = buildInitialGameState(config);
          setGameState(nextState, { render: false });
          initGoldenDeck();
          gameState.boardPath = BOARD_PATH_IDS.slice();
          ...
          var roomRes = await supabase
            .from("rooms")
            .insert({ code: lobbyCode, state: snapshotState(gameState) })
            .select()
            .single();
          if (roomRes.error) {
            console.error("room insert error", roomRes.error);
            alert("방을 생성할 수 없습니다. 콘솔을 확인하세요.");
            return;
          }

          var hostPlayer = gameState.players[0] || {
          ...
          var playerRes = await supabase
            ...
            .insert({
              ...
              cash: hostPlayer.money,
              position: hostPlayer.position,
            })
            .select()
            .single();
          if (playerRes.error) {
            console.error("player insert error", playerRes.error);
            alert("플레이어 정보를 저장할 수 없습니다.");
            return;
          }

          currentRoom = roomRes.data;
          currentPlayerRecord = playerRes.data;
          gameMode = "online";
          subscribeToRoom(currentRoom);
          refreshOnlinePlayers(currentRoom.id);
          updateOnlineStatusUI("방 " + lobbyCode + " 생성 완료");
          alert("방이 생성되었습니다! 코드 " + lobbyCode + " 를 공유하세요.");
        } catch (err) {
      ...
      async function joinRoom(lobbyCode, playerName) {
        ...
        try {
          ...
          if (existingRes.error) {
            console.error("players lookup error", existingRes.error);
            alert("플레이어 정보를 불러올 수 없습니다.");
            return;
          }
          var playerRow = existingRes.data && existingRes.data[0];
          if (!playerRow) {
            var remoteState = room.state || gameState;
            var desiredName = playerName || "플레이어";
            var seat = (remoteState.players || []).find(function (pl) {
              return pl.name === desiredName;
            });
            var insertPayload = {
              room_id: room.id,
              user_id: user.id,
              name: desiredName,
              cash: seat ? seat.money : 0,
              position: seat ? seat.position : 0,
            };
            var insertRes = await supabase
              ...
              .single();
            if (insertRes.error) {
              console.error("join player insert", insertRes.error);
          ...
          currentRoom = room;
          currentPlayerRecord = playerRow;
          gameMode = "online";
          setGameState(room.state || gameState);
          subscribeToRoom(room);
          refreshOnlinePlayers(room.id);
          updateOnlineStatusUI("방 " + lobbyCode + " 참여 중");
          alert("방에 입장했습니다.");
        } catch (err) {
      ...
      elRollBtn.addEventListener("click", async function () {
        var changed = rollDice();
        if (changed) {
          await syncOnlineState("roll");
        }
      });
      if (elActionBtn) {
        elActionBtn.addEventListener("click", openCurrentTileDetail);
      }
      elEndTurnBtn.addEventListener("click", async function () {
        var changed = endTurn();
        if (changed) {
          await syncOnlineState("endTurn");
        }
      });
      elBoardGrid.addEventListener("click", handleBoardClick);
      elSpaceGoBtn.addEventListener("click", async function () {
        var changed = handleSpaceGo();
        if (changed) {
          await syncOnlineState("space");
        }
      });
      if (elTileDetailActions) {
        elTileDetailActions.addEventListener("click", async function (e) {
          var btn = e.target.closest("[data-action]");
          if (!btn) return;
          var idx = parseInt(btn.dataset.index, 10);
          var changed = false;
          if (btn.dataset.action === "buy") {
            changed = attemptPurchase(idx);
          } else if (btn.dataset.action === "build") {
            changed = attemptBuild(idx);
          }
          if (changed) {
            await syncOnlineState("tile-action");
          }
        });
      }
